...
<ol class="${menuType} collapsibleContent"><!--
    <c:forEach var="child" items="${parentNode.children}">
        <c:if test="${(child.page || child.parent) && !child.system}">
            --><li class="menuelement <c:if test='${child.active}'>active</c:if> <c:if test='${(child.parent) && (child.page)}'>multilevel</c:if>">
            <c:choose>
                ...
                <c:when test="${child.page && child.parent}">
                    ...
                    <a href="#" class="caret menuarrow"><i>${resourceBundle.getString("label.children")}</i></a>
                </c:when>
                ...
                <c:when test="${!child.page && child.parent}">
                    <a href="${child.URI}" class="menuarrow"><span>${child.name}</span><i class="caret">${resourceBundle.getString("label.children")}</i></a>
                </c:when>
